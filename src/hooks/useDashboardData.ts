...
export function useDashboardData() {
  ...
  const osDistribuicaoPorTipo = useMemo(() => {
    ...
    ordensServico.forEach(os => { counts[os.tipo] = (counts[os.tipo] || 0) + 1; });
  ...
  const osDistribuicaoPorStatus = useMemo(() => {
    if (!ordensServico) return [];
    const counts: Record<string, number> = {};
    ordensServico.forEach(os => { counts[os.status] = (counts[os.status] || 0) + 1; });
    return Object.entries(counts).map(([status, quantidade]) => ({ status, quantidade }));
  }, [ordensServico]);
  ...
  const backlogStats = useMemo(() => {
    ...
    const atrasadas = backlogOS.filter(os => {
      ...
      return differenceInDays(now, dataSolicitacao) > 7 && os.status === 'ABERTA';
    }).length;
  ...
  const taxaCorretivaPreventiva = useMemo(() => {
    ...
    const osDoMes = ordensServico.filter(os => parseISO(os.data_solicitacao) >= start);
    ...
    return { corretivas, preventivas, ratio: total > 0 ? (preventivas / total) * 100 : 0 };
  }, [ordensServico]);
