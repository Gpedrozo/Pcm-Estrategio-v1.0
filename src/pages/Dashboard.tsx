...
import { FileText, FilePlus, FileCheck, Clock, Activity, Gauge, Target, TrendingUp, DollarSign, Calendar } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Bem-vindo, {user?.nome}! Visão geral da manutenção industrial.</p>
        </div>
        <Link to="/os/nova">
          <Button className="gap-2"><FilePlus className="h-4 w-4" />Nova O.S</Button>
        </Link>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPIGaugeCard title="MTBF" value={ind.mtbf} unit="h" min={0} max={1000} target={720} icon={Clock} description="Tempo Médio Entre Falhas" />
          <KPIGaugeCard title="MTTR" value={ind.mttr} unit="h" min={0} max={8} target={2} thresholds={{ warning: 4, critical: 6 }} icon={Activity} description="Tempo Médio Para Reparo" />
          <KPIGaugeCard title="Disponibilidade" value={ind.disponibilidade} unit="%" min={0} max={100} target={95} icon={Gauge} description="MTBF / (MTBF + MTTR)" />
          <KPIGaugeCard title="Aderência PM" value={aderenciaPreventiva} unit="%" min={0} max={100} target={90} icon={Target} description="Execução de Preventivas" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <p className="text-2xl font-bold font-mono text-foreground">{formatCurrency(ind.custoTotalMes)}</p>
          <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
            <div><p className="text-muted-foreground">M.O</p><p className="font-medium text-info">{formatCurrency(ind.custoMaoObraMes)}</p></div>
            <div><p className="text-muted-foreground">Mat.</p><p className="font-medium text-success">{formatCurrency(ind.custoMateriaisMes)}</p></div>
            <div><p className="text-muted-foreground">Terc.</p><p className="font-medium text-warning">{formatCurrency(ind.custoTerceirosMes)}</p></div>
          </div>
        </div>

        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 rounded-lg bg-success/10"><TrendingUp className="h-5 w-5 text-success" /></div>
            <span className="text-sm font-medium text-muted-foreground">Ratio Prev/Corr</span>
          </div>
          <p className="text-2xl font-bold font-mono text-foreground">{taxaCorretivaPreventiva.ratio.toFixed(0)}%</p>
          <p className="text-xs text-muted-foreground mt-1">Preventivas este mês</p>
          <div className="mt-3 flex gap-4 text-xs">
            <div><p className="text-muted-foreground">Preventivas</p><p className="font-medium text-success">{taxaCorretivaPreventiva.preventivas}</p></div>
            <div><p className="text-muted-foreground">Corretivas</p><p className="font-medium text-destructive">{taxaCorretivaPreventiva.corretivas}</p></div>
          </div>
        </div>

        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 rounded-lg bg-info/10"><Calendar className="h-5 w-5 text-info" /></div>
            <span className="text-sm font-medium text-muted-foreground">Backlog (Semanas)</span>
          </div>
          <p className="text-2xl font-bold font-mono text-foreground">{ind.backlogSemanas.toFixed(1)}</p>
          <p className="text-xs text-muted-foreground mt-1">{ind.backlogQuantidade} OS • {ind.backlogTempo.toFixed(0)}h acumuladas</p>
          <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${ind.backlogSemanas <= 2 ? 'bg-success' : ind.backlogSemanas <= 4 ? 'bg-warning' : 'bg-destructive'}`}
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 rounded-lg bg-warning/10"><Activity className="h-5 w-5 text-warning" /></div>
            <span className="text-sm font-medium text-muted-foreground">Aderência Prog.</span>
          </div>
          <p className="text-2xl font-bold font-mono text-foreground">{ind.aderenciaProgramacao.toFixed(0)}%</p>
          <p className="text-xs text-muted-foreground mt-1">Execução vs Programação</p>
          <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all ${ind.aderenciaProgramacao >= 90 ? 'bg-success' : ind.aderenciaProgramacao >= 70 ? 'bg-warning' : 'bg-destructive'}`}
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="table-industrial">
              <thead>
                <tr>
                  <th>Nº O.S</th><th>TAG</th><th>Equipamento</th><th>Tipo</th><th>Prioridade</th><th>Status</th><th>Data</th>
                </tr>
              </thead>
              <tbody>
                {osRecentes && osRecentes.length > 0 ? (
                  osRecentes.map((os) => (
                    <tr key={os.id}>
                      ...
                      <td><OSTypeBadge tipo={os.tipo as TipoOS} /></td>
                      <td>
                      ...
                      <td><OSStatusBadge status={os.status as StatusOS} /></td>
                      <td className="text-muted-foreground">{formatDate(os.data_solicitacao)}</td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan={7} className="text-center text-muted-foreground py-8">Nenhuma ordem de serviço cadastrada</td></tr>
                )}
              </tbody>
            </table>
          </div>
