...
export function BacklogSummary({ backlogQuantidade, backlogTempo, backlogSemanas, urgentes, atrasadas, metaSemanas = 2 }: BacklogSummaryProps) {
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2"><Clock className="h-5 w-5" />Backlog de Manutenção</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className={cn("font-medium", isHealthy ? "text-success" : "text-destructive")}>{percentMeta.toFixed(0)}%</span>
          </div>
          <Progress value={Math.min(percentMeta, 100)} className={cn("h-2", !isHealthy && "[&>div]:bg-destructive")} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-lg border border-border">
            <div className="flex items-center gap-2 text-muted-foreground mb-1"><TrendingDown className="h-4 w-4" /><span className="text-xs">Total OS</span></div>
            <p className="text-xl font-bold">{backlogQuantidade}</p>
          </div>
          <div className="p-3 rounded-lg border border-border">
            <div className="flex items-center gap-2 text-muted-foreground mb-1"><Clock className="h-4 w-4" /><span className="text-xs">Horas Acum.</span></div>
          ...
          <div className="space-y-2">
            {urgentes > 0 && (
              <div className="flex items-center justify-between p-2 rounded bg-destructive/10 border border-destructive/20">
                <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-destructive" /><span className="text-sm text-destructive font-medium">Urgentes</span></div>
            ...
            {atrasadas > 0 && (
              <div className="flex items-center justify-between p-2 rounded bg-warning/10 border border-warning/20">
                <div className="flex items-center gap-2"><Clock className="h-4 w-4 text-warning" /><span className="text-sm text-warning font-medium">Atrasadas</span></div>
